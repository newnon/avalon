...
- (void)paymentQueue:(SKPaymentQueue *)queue restoreCompletedTransactionsFailedWithError:(NSError *)error
{
    ...
    if (manager && manager->delegate) {
        manager->delegate->onPurchaseFail(manager, [BackendMac convertErrorToCode:error]);
    }
...
{
    if (manager && manager->delegate) {
        manager->delegate->onPurchaseFail(manager, [BackendMac convertErrorToCode:transaction.error]);
    }
