...
, config()
{
    ...
    for (auto& section : *config.getSections()) {
        ...
        if (!config.getSection(sectionName)->count("type")) {
            AVALON_ASSERT_MSG(false, "Product has no type defined");
            continue;
        }
        ...
        if (productId == NULL) {
            AVALON_ASSERT_MSG(false, "Product has no productId set for this platform");
            continue;
        }
        ...
        if (strcmp(type, "non-consumable") == 0) {
            ...
            AVALON_ASSERT_MSG(quantity > 0, "Quantity missing or set to zero");

            product = new ProductConsumable(productId, quantity);
        } else {
            AVALON_ASSERT_MSG(false, "Couldn't identify product type from ini file");
            continue;
        }
