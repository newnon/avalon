...
import com.avalon.payment.BackendGoogle;

public class PurchasingObserverGoogle implements OnActivityResultListener
{
    ...
    public PurchasingObserverGoogle()
    {
    ...
    private void threadDelegateOnServiceStarted()
    {
        Cocos2dxHelper.runOnGLThread(new Runnable() {
            @Override
            public void run() {
            	BackendGoogle.delegateOnServiceStarted();
            }
        });
    }
    ...
    private void threadDelegateOnPurchaseFail()
    {
        Cocos2dxHelper.runOnGLThread(new Runnable() {
            @Override
            public void run() {
            	BackendGoogle.delegateOnPurchaseFail();
            }
        });
    }
    ...
    private void threadDelegateOnPurchaseSucceed(final String sku)
    {
        Cocos2dxHelper.runOnGLThread(new Runnable() {
            @Override
            public void run() {
            	BackendGoogle.delegateOnPurchaseSucceed(sku);
            }
        });
    }
    ...
    private void threadDecrementTaskCounter()
    {
        if (--taskCount == 0) {
            Cocos2dxHelper.runOnGLThread(new Runnable() {
                @Override
                public void run() {
                	BackendGoogle.delegateOnTransactionEnd();
                }
            });
        }
    }
    ...
    private void threadIncrementTaskCounter()
    {
        if (++taskCount == 1) {
            Cocos2dxHelper.runOnGLThread(new Runnable() {
                @Override
                public void run() {
                	BackendGoogle.delegateOnTransactionStart();
                }
            });
        }
    }
        ...
        Cocos2dxHelper.runOnGLThread(new Runnable() {
            ...
            public void run() {
                for (String sku : inventory.getAllDetailsSkus()) {
                    ...
                    BackendGoogle.delegateOnItemData(
                        details.getSku(),
